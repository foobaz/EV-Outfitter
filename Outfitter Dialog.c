...
void Edit( FSSpec* gameFile )
{
	...
	page = 197;
		...
		/* Carry out the userÕs every desire until he saves or cancels */
	do
	{
		iHit = 0;
		do
...
void SavePage()
{
	...
	if( page == 0 )
	{
		...
		if( howMany == 28 )
			howMany = 64;
		else if( howMany == 27 )	// after calculations, howMany is 0 to bypass theÉ
			howMany = 4661;	// Éwrite-to-file. I didnÕt create ship and wonÕt mess with it.
		howMany += 127;	// get ship resource ID form menu itemÉ
...
help:
			if( NoteAlert( 132, nil ) == 3 )
				goto about;
			break;
		case 0x20002:	// cancelÉ
			dialUp = false;
			break;
		case 0x20004:	// save asÉ
			dialUp = SaveAs();
			break;
